...
tokensWithDecimalPlaces = (amount, symbol) => {
  amount = toFixed(amount).toString()
  switch (symbol) {
    case DAI: // 18 decimals
      return web3.utils.fromWei(amount, 'ether')
    case USDC:
      let v = (new web3.utils.BN(amount)).div(new web3.utils.BN('1000000'))
      return v.toString()
    default:
      return web3.utils.fromWei(amount, 'ether')
  }
...
async function checkArb(args) {
  ...
  let amountLeft = metadata.remainingFillableTakerAmount
  ...
  let inputAssetAmount = zrxOrder.takerAmount
  ...
  if(amountLeft != zrxOrder.takerAmount){
  ...
  const oneSplitData = await fetchOneSplitData({
    ...
    amount: zrxOrder.makerAmount,
  })

  // This becomes the outputAssetAmount
  // const outputAssetAmount = oneSplitData.returnAmount
  const outputAssetAmount = (new web3.utils.BN(oneSplitData.returnAmount)).mul(new web3.utils.BN('1000')).div(new web3.utils.BN('1000')).toString()

  // Calculate estimated gas cost
  let estimatedGasFee = process.env.ESTIMATED_GAS.toString() * web3.utils.toWei(process.env.GAS_PRICE.toString(), 'Gwei')

  let netProfit
  let _profit = outputAssetAmount - inputAssetAmount
  // console.table([{
  ...
  //   'Profit': displayTokens(_profit.toString(), assetOrder[0]).padEnd(22, ' '),
  //   'Timestamp': now(),
  // }])
  if (_profit > 0) {
    console.log("output > input", outputAssetAmount, inputAssetAmount, assetOrder)
  }

  if (zrxOrder.takerTokenFeeAmount.toString() !== '0') {
    ...
    if (zrxOrder.takerToken == ASSET_ADDRESSES[assetOrder[0]]) {
    	...
    	netProfit = outputAssetAmount - inputAssetAmount - estimatedGasFee - zrxOrder.takerTokenFeeAmount
  	...
  	netProfit = outputAssetAmount - inputAssetAmount
  }
...
async function trade(flashTokenSymbol, flashTokenAddress, arbTokenAddress, orderJson, fillAmount, oneSplitData) {
  const FLASH_AMOUNT = toTokens('15000', flashTokenSymbol) // 10,000 WETH
  ...
  let makerAssetData = '0xf47261b0000000000000000000000000' + ltrim(orderJson.makerToken, '0x')
  let takerAssetData = '0xf47261b0000000000000000000000000' + ltrim(orderJson.takerToken, '0x')
  // TODO remove duplication
  const orderTuple = [
    orderJson.maker,
    orderJson.taker,
    orderJson.feeRecipient,
    orderJson.sender,
    orderJson.makerAmount ,
    orderJson.takerAmount ,
    // orderJson.makerFee ,
    '0',
    orderJson.takerTokenFeeAmount,
    orderJson.expiry,
    orderJson.salt ,
  ...
  let sign = orderJson.signature
  const signature = sign.r  + ltrim(sign.s, '0x') + sign.v + '0' + sign.signatureType
...
async function checkOrderBook(baseAssetSymbol, quoteAssetSymbol) {
  ...
  let url = `https://api.0x.org/sra/v4/orderbook?baseToken=0x${baseAssetAddress}&quoteToken=0x${quoteAssetAddress}&perPage=500`
  const zrxResponse = await axios.get(url)
